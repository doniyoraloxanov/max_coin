    ...
    if (ctx.match && ctx.match.includes("refer")) {
      await ctx.replyWithHTML(
        ctx.t("excitementMess", {
          link: `${process.env.NEXT_PUBLIC_BOT_USERNAME}?start=${ctx.from?.id}`,
        }),
        {
          reply_markup: new InlineKeyboard().url(
            "Share Referral Link",
            `https://t.me/share/url?text=🎁 Join MaxCoin and start Earning 🎁🎁🎁 on MaxCoin&url=${process.env.NEXT_PUBLIC_BOT_USERNAME}?start=${ctx.from?.id}`
          ),
        }
      );
